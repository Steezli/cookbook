-- Public author attribution RPCs
-- SECURITY DEFINER allows anon callers to read display_name from profiles
-- without granting direct SELECT on profiles table.

-- Single recipe author lookup
...
grant execute on function get_public_recipe_author(uuid) to anon;

-- Batch recipe authors lookup
...
grant execute on function get_public_recipe_authors(uuid[]) to anon;
