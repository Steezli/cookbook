...
plan: 03
subsystem: ui
tags: [expo-router, expo-router-ui, navigation, tabs, sidebar, tab-bar, lucide, react-native-safe-area-context]

# Dependency graph
requires:
  - phase: 09-01
    provides: lucide-react-native installed, nav types (TabDestination/SidebarDestination), tokens.ts
  - phase: 09-02
    provides: (tabs)/_layout.tsx with headless Tabs and placeholder nav chrome, all screens in (tabs)/
...
affects:
  - 10-home-screen (uses PageContainer and tab navigation chrome built here)
  - 11-public-browsing (depends on nav chrome being complete)
  - 12-all-screens (all screens use PageContainer and navigate through this chrome)

# Tech tracking
tech-stack:
  added: []
  patterns:
    - "TabButton forwardRef with TabTriggerSlotProps: exposes isFocused for color switching, forwards onPress/onLongPress from TabTrigger asChild"
    - "SidebarItem forwardRef with TabTriggerSlotProps: same pattern as TabButton, adds label text and active bg styling"
    - "Scan button as plain Pressable: avoids TabTrigger onPress ambiguity for modal interception"
    - "Non-tab sidebar items (Collections, Scan) as plain SidebarItem with onPress + router.push"
    - "Breakpoint gate for nav chrome: isWeb === breakpoint === 'web'; tablet uses MobileTabBar"
