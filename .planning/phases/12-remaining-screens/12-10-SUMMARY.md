...
plan: 10
subsystem: ui
tags: [react-native, auth, collections, web-compat, supabase-dashboard]

# Dependency graph
requires:
  - phase: 12-remaining-screens
    provides: family detail confirmAction pattern (12-08), collections routing (12-07)
provides:
...
affects: [signup, collections-detail, forgot-password]

# Tech tracking
tech-stack:
  added: []
  patterns:
    - "confirmAction/showAlert helpers in collections/[id].tsx: Platform.OS branch — window.confirm on web, Alert.alert on native — matching family detail pattern from 12-08"
    - "Inline text link for cross-screen auth nav: accentWarm Text inside a Link, no Pressable border — matches login screen Sign Up link pattern"
...
requirements-completed: [SCREEN-05, SCREEN-08]
